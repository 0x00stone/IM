...
import static UI.controller.View.index.getIndex;
import static UI.controller.View.index.saveIndex;
...
public static user[] UpdataNickname(Socket socket,TextArea textArea) throws IOException {
    ...
    try{
        if(new File("./Config2.properties").exists()){
            FileInputStream fis = new FileInputStream(new File("./Config2.properties"));
            ...
            FileOutputStream fos = new FileOutputStream(new File("./Config2.properties"));
            Properties Config = new Properties();
            addName(Config,textOut,textin,button);
